...
const WalletConnectComponent = () => {
  ...
  const connectorButtons = (
    <div className="d-flex gap-2 flex-wrap mb-3">
      <button className={`btn btn-sm ${connector==='metamask'?'btn-primary':'btn-outline-primary'}`} onClick={()=>setConnector('metamask')}>
        <i className="fas fa-fox me-1"></i> MetaMask
      </button>
      <button className={`btn btn-sm ${connector==='walletconnect'?'btn-primary':'btn-outline-primary'}`} onClick={()=>setConnector('walletconnect')}>
        <i className="fas fa-qrcode me-1"></i> WalletConnect
      </button>
      <button className={`btn btn-sm ${connector==='coinbase'?'btn-primary':'btn-outline-primary'}`} onClick={()=>setConnector('coinbase')}>
        <i className="fas fa-coins me-1"></i> Coinbase Wallet
      </button>
    </div>
  );
  ...
  return (
    <div className="wallet-connect-container">
      <div className="card mb-3">
        <div className="card-header"><h5 className="mb-0"><i className="fas fa-wallet me-2"></i>Wallet Connection</h5></div>
          ...
          {!walletState.isConnected ? (
            ...
            <div>
              ...
              <div className="mb-3">
                ...
                <div className="d-flex gap-2 flex-wrap">
                  {Object.entries(SUPPORTED_NETWORKS).map(([id, n]) => (
                    <button key={id} className={`btn btn-sm ${walletState.chainId===Number(id)?'btn-primary':'btn-outline-primary'}`} onClick={()=>handleSwitchNetwork(Number(id))} disabled={isSwitching}>
                      {n.name}
                    </button>
                  ))}
                </div>
              ...
              <div className="d-flex gap-2 mb-3">
                <button className="btn btn-success" onClick={handleTestTransaction} disabled={!!(pendingTx && ['pending','confirming'].includes(pendingTx.status))}>
                  <i className="fas fa-rocket me-2"></i>Test Transaction
                </button>
                <button className="btn btn-outline-danger" onClick={handleDisconnect}><i className="fas fa-times me-2"></i>Disconnect</button>
              </div>
            </div>
          )}
        </div>
      ...
      {pendingTx && (
        <div className="card">
          <div className="card-header"><h6 className="mb-0"><i className="fas fa-clock me-2"></i>Transaction Status</h6></div>
